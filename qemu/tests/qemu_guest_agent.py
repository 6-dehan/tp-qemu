...
class QemuGuestAgentBasicCheckWin(QemuGuestAgentBasicCheck):
    ...
    def gagent_check_resource_leak(self, test, params, env):
        ...
        def _process_qga_resource():
            """
            Check the handles of process qemu-ga.
        ...
        def _base_on_bg_check_resource_leak():
            ...
            if bg.is_alive():
                ...
                if qga_handles > 300 or qga_memory > 3000:
                    ...
                    logging.info("Current qga_handls is %s, qga_memory is "
                                 "%s" % (qga_handles, qga_memory))
                return False
            else:
                return True
        ...
        error_context.context("Check whether resources leak after executing"
                              " get-osinfo/devices in a loop.", logging.info)
        repeats = int(params.get("repeat_times", 1))

        bg = utils_misc.InterruptedThread(execute_qga_cmds_loop)
        bg.start()
        utils_misc.wait_for(lambda: _base_on_bg_check_resource_leak(), 180)
